...
ONE_DESCRIPTOR CustomHID_KeyBorad_Report_Descriptor =
{
    (uint8_t *)CustomHID_KeyBoardReportDescriptor,
    CUSTOMHID_KEYBOARD_SIZ_REPORT_DESC
};
...
ONE_DESCRIPTOR CustomHID_KeyBorad_Descriptor =
{
    (uint8_t*)CustomHID_ConfigDescriptor + CUSTOMHID_OFF_KEYBOARD_HID_DESC,
    CUSTOMHID_SIZ_HID_DESC
};
...
//		 SetEPRxAddr(ENDP3, ENDP3_RXADDR); //设置接收数据的地址
//		SetEPRxCount(ENDP3, 2);  //设置接收长度
//		SetEPRxStatus(ENDP3, EP_RX_VALID); //设置端点有效，可以接收数据
//		
      ...
      && (pInformation->USBwIndex0 < 2))
  {
    ...
    if (pInformation->USBwValue1 == REPORT_DESCRIPTOR)
    {
      ...
      CopyRoutine = CustomHID_GetReportDescriptor;
			...
			CopyRoutine = CustomHID_GetKeyBoardReportDescriptor;
    }
    else if (pInformation->USBwValue1 == HID_DESCRIPTOR_TYPE)
    {
      ...
      CopyRoutine = CustomHID_GetHIDDescriptor;
			 ...
			 CopyRoutine = CustomHID_GetHIDKeyBoardDescriptor;
    }
...
uint8_t *CustomHID_GetKeyBoardReportDescriptor(uint16_t Length)
{
  return Standard_GetDescriptorData(Length, &CustomHID_KeyBorad_Report_Descriptor);
}
...
uint8_t *CustomHID_GetHIDKeyBoardDescriptor(uint16_t Length)
{
  return Standard_GetDescriptorData(Length, &CustomHID_KeyBorad_Descriptor);
}
